...
master_file = "First_Run_Heroku.tsv"
...
def append_to_file_insert_to_db(file_name, entry_list, main_dict):
    calls_made = 0
    ...
    with open(temp_path + file_name, 'ab') as gnw:
        ...
        for entry in entry_list:
            ...
            if check is True:
                ...
                '{9}','{10}')'''.format(line[0], line[1], line[2], line[3], line[4].replace("'", "''"), line[5],
                                        line[6].replace("'", "''"), line[7], line[8], line[9],
                                        line[10].replace("'", "''"))
...
with open(test_path + master_file, 'rb') as R:
    ...
    for row in reader:
        ...
        ppfa_gnw_jds_row = [unique_code, internal, matched_rollup, "PS-CharlesMaxwellScofieldBoyd", job_desc, industry, \
                            short_task_string]
...
append_to_file_insert_to_db("pfaa-gnw-jds.csv", entries_to_use, gnw_jds)
